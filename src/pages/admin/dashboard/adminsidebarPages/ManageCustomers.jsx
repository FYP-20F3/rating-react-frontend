...
const CustomerTable = ({ data, deleteCustomer, blockCustomer }) => {
  ...
  const sortedData = data.slice().sort((a, b) => {
    switch (orderBy) {
      case "createdAt":
        return order === "asc"
          ? moment.utc(a.createdAt).local().diff(moment.utc(b.createdAt).local())
          : moment.utc(b.createdAt).local().diff(moment.utc(a.createdAt).local());
      case "blockTimeStamp":
        return order === "asc"
          ? moment.utc(a.blockTimeStamp).local().diff(moment.utc(b.blockTimeStamp).local())
          : moment.utc(b.blockTimeStamp).local().diff(moment.utc(a.blockTimeStamp).local());
      case "block":
        return order === "asc"
          ...
          <TableBody>
            {(rowsPerPage > 0
              ...
              <TableRow key={row._id}>
                ...
                <TableCell className="tw-py-3 tw-px-4">
                  {row.blockTimeStamp
                    ? moment.utc(row.blockTimeStamp).format("MM/DD/YYYY")
                    : "-"}
                </TableCell>
                <TableCell className="tw-py-3 tw-px-4">
                  <Chip
