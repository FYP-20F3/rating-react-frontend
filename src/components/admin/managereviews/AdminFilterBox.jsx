...
import FilterBox from "../../customer/businessInfo/FilterBox";
  ...
  const fetchBusinessReviews = useCallback(
    async (params) => {
      try {
        ...
        // console.log(`${BASE_URL}reviews/searchReviews/null?${queryString}`);
        const response = await axios.get(
          `${BASE_URL}reviews/searchReviews/null?${queryString}`,
          {
            headers: {
        ...
        if (filterChanged) {
          // console.log(`/admin/manage/reviews?${queryString}`);
          navigate(`/admin/manage/reviews?${queryString}`);
        }
      } catch (error) {
        console.log(error);
      }
    },
    [filterChanged, navigate, token]
  );
      ...
      <List className="tw-mt-4">
        {reviews.map((review, index) => (
          <ReviewBox
            key={index}
            review={review}
            businessName={review?.businessId?.businessName}
          />
        ))}
      </List>
    </div>
